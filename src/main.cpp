...
// nailong-pet —— 桌面宠物主程序
// 功能概览：
//   1. 透明无边框窗口，始终置顶，尺寸自适应素材分辨率
//   2. 帧动画系统：idle / smile / laugh / cry 四套动画，12fps 播放
//   3. 点击触发微笑动画，1.5秒内连点5次触发大笑动画，配套音效
//   4. 快速甩动后触发物理飞行：重力、空气阻力、边缘弹跳、碰撞形变动画
//   5. CPU/内存监控：过载（CPU>70% 或内存>80%）时触发哭泣状态
//   6. 连续使用60分钟后触发疲惫状态，显示"Take a break!"气泡
//   7. 程序启动和电脑唤醒时显示打招呼气泡（问候/日期/IP属地/陪伴时长）
//   8. 陪伴时长跨 session 持久化到本地文件
//   9. 音效系统：smile/laugh 播放真实 MP3，支持静音开关
//  10. 右键菜单：关闭/最小化到托盘/固定/禁用大笑/静音/刷新状态/系统信息
// ============================================================
...
// ============================================================
// 桌宠状态枚举
//   Normal  —— 默认状态，播放 idle 帧动画
//   Smile   —— 单击触发，播放 smile 帧动画，持续 750ms
//   Laugh   —— 连击5次触发，播放 laugh 帧动画，持续 5667ms，配套延迟音效
//   Tired   —— 连续使用超60分钟触发，播放 idle 动画 + "Take a break!"气泡
//   PreCry  —— 系统刚超过负载阈值时的过渡状态，1秒后自动切换到 Cry
//   Cry     —— CPU或内存过载时触发，播放 cry 帧动画循环
//   Drag    —— 鼠标拖动时触发，播放 idle 动画（与静止外观一致）
// ============================================================
